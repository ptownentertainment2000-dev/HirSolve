...
export async function requestWithdrawal(input: { session: AuthSession; stepUpToken: string; amount: string | number; currency: string; destinationRef: string; idempotencyKey: string; availableAccountId: string; payableAccountId: string }) {
  ...
  if (!amount.isFinite() || amount.lte(0)) throw new WithdrawalError('Withdrawal amount must be positive');

  return prisma.$transaction(async (tx) => {
    const existing = await tx.withdrawalRequest.findUnique({ where: { idempotencyKey: input.idempotencyKey } });
    ...
    // The balance check and ledger movement must use the same transaction boundary.
    const postings = await tx.ledgerPosting.findMany({ where: { accountId: input.availableAccountId, currency: input.currency, transaction: { status: 'POSTED' } }, select: { side: true, amount: true } });
    const balance = postings.reduce((total, posting) => posting.side === 'DEBIT' ? total.plus(posting.amount) : total.minus(posting.amount), new Prisma.Decimal(0));
    if (balance.lt(amount)) throw new WithdrawalError('Insufficient available balance');
